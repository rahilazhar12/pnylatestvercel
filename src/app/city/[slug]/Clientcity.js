                ...
                {data.special_pages && data.special_pages.length > 0 ? (
                    data.special_pages.map((item) => {
                        return (
                            <div
                                key={item.url_slug}
                                className={`block rounded-lg bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] ${item.name === "Short Courses in Lahore" ? "h-52" : "h-auto"
                                    }`}
                            >
